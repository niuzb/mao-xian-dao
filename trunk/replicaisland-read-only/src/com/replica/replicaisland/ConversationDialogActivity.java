...
public class ConversationDialogActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.v("replica", "level row:"+levelRow+" levelIndex"+levelIndex + "index"+index+character);
        mPages = null;
    ...
    private void formatPages(Conversation conversation, TextView textView) {
        ...
        for (int page = conversation.pages.size() - 1; page >= 0 ; page--) {
        	...
        	int addedPages = 0;
        	int lastPageStart = 0;
            //Log.v("replica","page:"+page+"maxLinesPerPage"+maxLinesPerPage);
            count=0;
            last = 0;
            returnc = 0;
        	do {
	        	int fittingChars = paint.breakText(text, currentOffset, textLength, true, maxWidth, null);
	        	//Log.v("replica","currentOffset:"+currentOffset+"fittingChars:"+fittingChars);
	        	if (currentOffset + fittingChars < textLength) {
	        		fittingChars -= 2;
	        		// Text doesn't fit on the line.  Insert a return after the last space.
	        		    int lastSpace;
                        lastSpace = TextUtils.lastIndexOf(text, ',', currentOffset + fittingChars - 1);
                        
                        if (lastSpace == -1 ||
                            lastSpace == last) {
                            //Log.v("replica","no comma lastSpace:"+lastSpace);
    	        			// No spaces, just split at the last character.
    	        			lastSpace = currentOffset + fittingChars;
                            //Log.v("replica","no comma lastSpace:"+lastSpace);
                            spannedText.replace(lastSpace, lastSpace, newline, 0, 1);
                            currentOffset = lastSpace;
                            returnc++;
                            
    	        		} else {
    	        		    //Log.v("replica","comma lastSpace:"+lastSpace);
                            spannedText.replace(lastSpace+returnc, lastSpace+returnc+1, newline, 0, 1);
